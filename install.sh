#!/usr/bin/env bash
# patent-search one-command installer
...
# ~/.agents/skills/patent-search (picked up by Cursor, Grok Build, and other
# file-based hosts). Per-host failures are non-fatal.
#
# Scripts are Python-3 stdlib only — no extra runtime dependency to install.
set -u

REPO="djfksjd/patent-search-skill"
REPO_URL="https://github.com/${REPO}.git"
INSTALLED=0

log()  { printf '\033[1;32m[patent-search]\033[0m %s\n' "$*"; }
warn() { printf '\033[1;33m[patent-search]\033[0m %s\n' "$*" >&2; }
...
if command -v claude >/dev/null 2>&1; then
  try_host "Claude Code" bash -c \
    "claude plugin marketplace add ${REPO} && claude plugin install patent-search@djfksjd"
fi

if command -v codex >/dev/null 2>&1; then
  try_host "Codex" bash -c \
    "codex plugin marketplace add ${REPO} && codex plugin add patent-search@djfksjd"
fi

if command -v agy >/dev/null 2>&1; then
  try_host "agy (Antigravity CLI)" bash -c \
    "agy plugin install ${REPO} && agy plugin enable patent-search"
fi
...
SKILL_DIR="${HOME}/.agents/skills/patent-search"
if [ -d "${SKILL_DIR}/.git" ]; then
  if git -C "${SKILL_DIR}" pull --ff-only >/dev/null 2>&1; then
    log "✓ ~/.agents/skills/patent-search 갱신 (Cursor·Grok Build 등)"
    INSTALLED=$((INSTALLED + 1))  # an update is a successful install too
  else
    warn "✗ ~/.agents/skills/patent-search 갱신 실패 — 수동으로 git pull 하세요"
  fi
elif [ ! -e "${SKILL_DIR}" ]; then
  mkdir -p "${HOME}/.agents/skills"
  if git clone --quiet "${REPO_URL}" "${SKILL_DIR}"; then
    log "✓ ~/.agents/skills/patent-search clone (Cursor·Grok Build 등)"
    INSTALLED=$((INSTALLED + 1))
  else
...
log "완료 — 새 세션에서 '선행기술조사 해줘'로 사용하세요."
