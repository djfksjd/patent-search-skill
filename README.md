...
무료 쿼터는 **월 1,000회 호출**(매월 1일 초기화, 2026-07 기준)이며, 스킬은 쿼터를 아끼도록 설계되어 있습니다(검색은 검색식당 1회, 청구항 상세는 정독 대상만).
...
- **종료 코드**: 0 = 전건 성공, 1 = 하나 이상 실패. 검색이 전체 건수를 다 못 담으면 매니페스트에 `partial=true`로 표시됩니다 — 부분 수집을 완전 수집으로 오인하지 않도록 설계했습니다.
